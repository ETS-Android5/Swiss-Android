...
class Customer with ChangeNotifier {
  ...
  List<ProblemType> _dropListModel = [];
  List<SupportVisitType> _supportvisittype = [];
  List<SerialNumber> _serialNumber = [];
  List<Dish> _dish = [];
  List<ChartData> _chart = [];
  List<WorkOrder> _workOrder = [];
  List<WorkOrder> _workOrderId = [];
  ...
  List<ProblemType> get problemTypes {
    return [..._dropListModel];
  }

  List<SupportVisitType> get supportVisitTypes {
    return [..._supportvisittype];
  }
  ...
  List<Dish> get allprods {
    return [..._dish];
  }

  List<ChartData> get dashboardData {
    return [..._chart];
  }

  List<WorkOrder> get listsWorkOrders {
    return [..._workOrder];
  }
      ...
      final categoriesList = data
          .map((json) => Category(
                ...
                image: json['image'] != null &&
                    ...
                    ? 'https://swissmed.deumsoft.com/assets/category/${json['image']}'
                    : '',
              ))
          .toList();
  ...
  Future<void> getSubCategories(String catId) async {
    ...
    try {
      ...
      //final responseData = json.decode(response.body.toString()) as Map<String, dynamic>;
      if (responseData['message'] != 'Get all Categories') {
        return null;
      }
  ...
  Future<void> getAllProblemTypes() async {
    final url = '$base_url/getAllProblemTypes';
    try {
      final response = await http.post(
        url,
        body: {
      ...
      final responseData =
          json.decode(response.body.toString()) as Map<String, dynamic>;
      if (responseData['message'] != 'Get All Problems Type') {
        return;
      }
      final data = List<Map<String, dynamic>>.from(responseData['data']);
      final problemTypes = data
          .map((json) => ProblemType(
              id: json['id'],
              problemType: json['ProblemType'],
              createdat: json['created_at'],
              updatedat: json['updated_at']))
          .toList();
      _dropListModel = problemTypes;
      notifyListeners();

      print('res type: ${problemTypes.length} ${responseData['data']}');
    } catch (err) {
      throw err;
    }
  }

  Future<void> getAllSupportVisitType() async {
    final url = '$base_url/getAllSupportVisitType';
    try {
      final response = await http.post(
        url,
        body: {
      ...
      final responseData =
          json.decode(response.body.toString()) as Map<String, dynamic>;
      if (responseData['message'] != 'Get All Support Visit Type') {
        return;
      }
      final data = List<Map<String, dynamic>>.from(responseData['data']);
      final supportVisitTypes = data
          .map((json) => SupportVisitType(
              id: json['id'],
              supportVisitType: json['SupportVisitType'],
              createdat: json['created_at'],
              updatedat: json['updated_at']))
          .toList();
      _supportvisittype = supportVisitTypes;
      notifyListeners();

      print('res type: ${supportVisitTypes.length} ${responseData['data']}');
    } catch (err) {
      throw err;
    }
  }

  Future<void> getAllProdDropDown() async {
  ...
  Future<void> getAllProds() async {
    final url = '$base_url/getAllProds';
    try {
      final response = await http.post(
        url,
        body: {
      ...
      final responseData =
          json.decode(response.body.toString()) as Map<String, dynamic>;
      if (responseData['message'] != 'Get All Products') {
        return;
      }
      final data = List<Map<String, dynamic>>.from(responseData['data']);
      final dish = data
      ...
      print('res type: ${dish.length} ${responseData['data']}');
    } catch (err) {
      throw err;
    }
  }

  Future<bool> addClientOrder(List<String> productList) async {
    var url = '$base_url/addClientOrder';
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: {'ClientID': _userId, 'productlist': productList.join(",")},
      );
      // print(productList);
      if (response.statusCode != 200) {
        return false;
  ...
  Future<List<ChartData>> dashboardChart() async {
    final url = '$base_url/DashboardChart';
    try {
      final response = await http.post(
        url,
        body: {
  ...
  Future<List<WorkOrder>> listOfWork(String typeid) async {
    final url = '$base_url/ListsWorkOrders';
    try {
      final response = await http.post(
        url,
        body: {
  ...
  Future<List<WorkOrder>> workerListsWorkOrdersbyID(String workOrderId) async {
    final url = '$base_url/WorkerListsWorkOrdersbyID';
    try {
      final response = await http.post(
        url,
        body: {
      ...
      final responseData =
          json.decode(response.body.toString()) as Map<String, dynamic>;
      if (responseData['message'] != 'WorkOrder Detail') {
        return null;
      }
      final data = List<Map<String, dynamic>>.from(responseData['data']);
      final workOrdersId = data
          .map((json) => WorkOrder(
              workOrderId: json['WorkOrderID'],
              problemType: json['ProblemType'],
              supportVisitType: json['SupportVisitType'],
              ...
              totalAmount: json['total_amount']))
          .toList();
      _workOrderId = workOrdersId;
      notifyListeners();
      log('res type: ${workOrdersId.length} ${responseData['data']}');
      return _workOrderId;
    } catch (err) {
      throw err;
    }
